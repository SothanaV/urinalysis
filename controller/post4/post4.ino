...
void GenWave(float vmin,float vmax,float vcc,float pw, float t,float a,float e)
{
    float v = vmax;
      while(1)
      {
        v = v-a; if(v<vmin) break;
        int x = (int)((v/vcc)*4095.0f);
        Serial.print(x);
        Serial.println("  ");
        dac.setVoltage(x, false);
        delay(pw);
        uint16_t rawV1 = adc.read(MCP3208::SINGLE_0);
        uint16_t valV1 = adc.toAnalog(rawV1);
        uint16_t rawI1 = adc.read(MCP3208::SINGLE_1);
        uint16_t valI1 = adc.toAnalog(rawI1);
        Vstr = Vstr+valV1+",";
        Istr = Istr+valI1+",";
        v=v+(a-e); if(v<vmin) break;
        x = (int)((v/vcc)*4095.0f);
        Serial.println(x);
        dac.setVoltage(x, false);
        delay(t-pw);
